...
if (!$_POST) {
	...
	$get_topic_sql = "SELECT * FROM forum_topics WHERE topic_id = $saved_id;";
	$get_topic_res = mysqli_query($mysqli, $get_topic_sql) or die(mysqli_error($mysqli));
	// get record from topic posting table
	$get_post_sql = "SELECT * FROM forum_posts WHERE topic_id = $saved_id;";
...
else
{
	...
	$update_topic_sql = "UPDATE forum_topics SET topic_title = '".$clean_topic_title ."' WHERE topic_id =".$_SESSION['topic_id'];
	$update_topic_res = mysqli_query($mysqli, $update_topic_sql) or die(mysqli_error($mysqli));

	//create and issue the forum_post update
	$update_post_sql = "UPDATE forum_posts SET post_text='" .$clean_post_text."' WHERE topic_id= ".$_SESSION['topic_id'];
